function [modelStructure,tMin,phi]=FullReichardtBothArmDeriv(sampleFreq)
...
function response = HalfCorrDerivStructure(xtPlot,sampleFreq,tau)    
    ...
    arm1ND = sum(bsxfun(@times,xtPlot(:,1,:),flipud(tFiltNd)),1);
    arm2D = sum(bsxfun(@times,xtPlot(:,2,:),flipud(tFiltD)),1);
    
    response = arm1D.*arm2ND - arm2D.*arm1ND;
